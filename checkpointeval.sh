                   ...
                   echo -e  "------------------------------------------------------------------- Unassigned"  $PREFIX-${FONTNAME// /_}-${TRAINEDDATAFILE%.*}
                    ...
                    do
                        ...
                        if [[ $LSTMFNAME == *${FONTNAME// /_}* ]]; then
                            OMP_THREAD_LIMIT=1 tesseract ${LSTMFNAME%.*}.$IMGEXT  ${LSTMFNAME%.*}-${TRAINEDDATAFILE%.*} --psm 13 --oem 1  -l  ${TRAINEDDATAFILE%.*}  --tessdata-dir $SCRIPTPATH/data/$LANG/tessdata_fast/  -c page_separator=''   1>/dev/null 2>&1
